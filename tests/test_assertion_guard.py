...
def test_assertions_removed_is_flagged():
    d = _diff(
        "e2e/todo.spec.ts",
        ["    expect(count).toBe(1);", "    expect(label).toHaveText('1 item left');"],
...
def test_matcher_softened_is_flagged():
    d = _diff(
        "e2e/todo.spec.ts", ["    expect(count).toBe(1);"], ["    expect(count).toBeDefined();"]
    )
    k = _kinds(ag.scan_diff(d))
...
def test_value_change_same_matcher_is_clean():
    ...
    d = _diff("e2e/todo.spec.ts", ["    expect(count).toBe(1);"], ["    expect(count).toBe(2);"])
...
def test_non_test_file_is_ignored():
    d = _diff("src/app.ts", ["  expect(x).toBe(1);"], ["  test.skip('x', () => {});"])
    r = ag.scan_diff(d)
...
def test_inline_comment_does_not_inflate_removed_count():
    ...
    d = _diff(
        "e2e/todo.spec.ts", ["    expect(x).toBe(1); // was: expect(y).toBe(2)"], ["    // removed"]
    )
...
def test_inline_block_comment_does_not_inflate_removed_count():
    d = _diff(
        "e2e/todo.spec.ts", ["    expect(x).toBe(1); /* expect(y).toBe(2) */"], ["    // removed"]
    )
    f = ag.scan_diff(d)["findings"]
    assert [x["kind"] for x in f] == ["assertions-removed"]
...
def test_re_enabling_a_skipped_test_is_not_flagged():
    ...
    d = _diff(
        "e2e/todo.spec.ts", ["  test.skip('adds', async () => {"], ["  test('adds', async () => {"]
    )
    assert ag.scan_diff(d)["findings"] == []
...
def test_bare_pytest_assert_removal_is_counted():
    d = _diff(
        "tests/test_pay.py", ["    assert charge == 49", "    assert status == 'ok'"], ["    pass"]
    )
    f = ag.scan_diff(d)["findings"]
    assert [x["kind"] for x in f] == ["assertions-removed"]
...
def test_summary_counts_by_kind():
    ...
    d = _diff(
        "e2e/todo.spec.ts",
        ["  test('a', async () => {", "    expect(x).toBe(1);", "    expect(y).toEqual(2);"],
        ["  test.skip('a', async () => {"],
    )

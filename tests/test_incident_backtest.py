...
def test_incident_covered_by_req_trace():
    ...
    assert any(
        reason.startswith("req-trace:TMVC-1") for reason in row["reasons"]["e2e/todo.spec.ts"]
    )
...
def test_incident_is_a_blind_spot_when_nothing_traces_it():
    inc = {
        ...
        "requirements": ["RPT-3"],
        "journeys": [],
        "symptom": "csv export returned an empty file",
        "severity": 5,
    }
    r = ib.backtest([inc], FILES)
...
def test_symptom_overlap_is_a_catch_but_labelled():
    files = {"e2e/login.spec.ts": "test('user can login with email and password', async () => {});"}
    inc = {
        "id": "INC-4",
        "title": "login with email and password failed",
        "requirements": [],
        "journeys": [],
        "symptom": "user could not login with email and password",
        "severity": 1,
    }
    r = ib.backtest([inc], files)
    row = r["incidents"][0]
    assert row["covered"] is True
    assert any(
        reason.startswith("symptom-overlap") for reason in row["reasons"]["e2e/login.spec.ts"]
    )
...
def test_load_incidents_accepts_list_and_wrapped():
    ...
    as_wrapped = ib.load_incidents(
        json.dumps({"incidents": [{"id": "X", "requirements": ["A-1"]}]})
    )
